...
var Formular = /** @class */ (function () {
    ...
    Formular.prototype.registerEvents = function () {
        var _this = this;
        var db = new DB("Formular");
        //On choosing Formular tab
        $("body").on("click", "#formular", function () {
            ...
            db.getAll("Formular").then(function (resolve) {
        ...
        $("select[name='select_from_db']").change(function () {
            ...
            if (version) {
                ...
                for (var i = 0; i < result.values.length; i++) {
                    ...
                    if (result.values[i].selection_validation == "Mandatory") {
                        ...
                        var validate = $("input[name='" + name + "']").val();
        ...
        $("#load").click(function () {
            ...
            var version = $("input[name='version']").val();
            if (select && version) {
                var size = _this.result.values;
                var ver = _this.result.key + " version " + version;
                db.getRow(ver, "Versions").then(function (resolve) {
                    if (resolve) {
                        ...
                        for (var i = 0; i < size.length; i++) {
                            var name = _this.result.values[i].label;
                            var type = _this.result.values[i].selection_type;
        ...
        //on update
        $("#update_formular").click(function () {
            var values_to_save = [];
            var version = $("input[name='version']").val();
            var result = _this.result;
            var ver = result.key + " version " + version;
